...
const TelaDetalhesUsuario = (props) => {
  const [campoNome, setCampoNome] = useState(
    props.route.params?.usuario.nome || ""
  );
  const [campoEmail, setCampoEmail] = useState(
    props.route.params?.usuario.email || ""
  );
  ...
  const salvar = async () => {
    try {
      const usuario = {
        id_usuarios: props.route.params?.usuario.id_usuarios,
        nome: campoNome,
        email: campoEmail,
        senha: campoNovaSenha,
      };
      if (props.route.params?.usuario.id_usuarios) {
        await api.put("/usuarios", usuario);
      } else {
        await api.post("/usuarios", usuario);
      }
      alert("Dados salvos com sucesso!");
      props.navigation.navigate(TELAS.TELA_LISTA_USUARIOS, {
        refresh: +new Date(),
      });
    } catch (error) {
  ...
  const excluir = async () => {
    try {
      if (confirm("Tem certeza?")) {
        await api.delete(`/usuarios/${props.route.params.usuario.id_usuarios}`);
  ...
  return (
    <View style={{ padding: 16, gap: 16 }}>
      {props.route.params?.usuario.id_usuarios && (
        <CampoTextoCustomizado
          label="ID"
          value={props.route.params.usuario.id_usuarios}
          disabled
          readonly
        />
      )}
